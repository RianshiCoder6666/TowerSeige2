...
function draw() {
  ...
  text("Press the Space Key to attach the ball to Slingshot again.",50,60);
  ...
  if(ball.velocity.x > 2 && ball.position.x > 200){
    var position = [ball.position.x, ball.position.y];
    trajectory.push(position);
}

  for(var i=0; i<trajectory.length; i++){
      image(smokeImage, trajectory[i][0], this.trajectory[i][1]);
  }
